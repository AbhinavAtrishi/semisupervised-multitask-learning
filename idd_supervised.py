...
from nnutils import resnet, ttn, ccr_nets
from nnutils import loss_utils
from nnutils import task_decoders as decoders
from nnutils import depth_decoders

from data.idd_semisupervised import TrainData, ValData
...
# Data Specific
flags.DEFINE_float('supervised_pct', None, 'Percentage of supervised data to train on')
...
flags.DEFINE_float('lr1', 0.0001, 'Learning Rate for the supervised training')
flags.DEFINE_float('lr2', 0.00001, 'Learning Rate for unsupervised training')
...
flags.DEFINE_string('idd_name', None, 'Name of the IDD adapted model to be loaded')
flags.DEFINE_string('idd_epoch', None, 'Which epoch of the IDD adapted model to load?')

flags.DEFINE_string('resume_name', None, 'Resume training!')
...
flags.mark_flag_as_required('supervised_pct')
flags.mark_flag_as_required('base_name')
flags.mark_flag_as_required('idd_name')
flags.mark_flag_as_required('idd_epoch')
...
class UMATrainer:
    ...
    def load_models(self):
        ...
        self.base_resnet5.load_state_dict(torch.load(f'cachedir/models/{opts.idd_name}/r5_{opts.idd_epoch}.pth'))
    ...
    def init_optimizer(self):
        opts = self.opts
        self.train_params = list(self.base_resnet5.parameters())
        
        if opts.optimizer == 'adam':
            self.optimizer1 = optim.AdamW(self.train_params, lr=opts.lr1)
            self.optimizer2 = optim.AdamW(self.train_params, lr=opts.lr2)
        else:
            print('Using SGD optimizer')
            self.optimizer1 = optim.SGD(self.train_params, lr=opts.lr1)
            self.optimizer2 = optim.SGD(self.train_params, lr=opts.lr2)
            
        self.scaler1 = torch.cuda.amp.GradScaler(enabled=opts.use_amp)
        self.scaler2 = torch.cuda.amp.GradScaler(enabled=opts.use_amp)
    
    def init_dataset(self):
        opts = self.opts
        print('Full Dataset : ')
        self.train_data_full = TrainData(opts)
        self.val_data = ValData(opts)
        total_size = len(self.train_data_full)
        sampled_idx = np.random.choice(range(total_size), replace=False, size=int(opts.supervised_pct*total_size))
        print(f'Training on {opts.supervised_pct * 100}% data, {int(opts.supervised_pct*total_size)} images.')
        sampled_dataset = torch.utils.data.Subset(self.train_data_full, indices=sampled_idx)
        self.trainloader = torch.utils.data.DataLoader(sampled_dataset, batch_size=opts.batch_size, shuffle=opts.shuffle_data, num_workers=opts.data_threads, pin_memory=opts.pin_memory, drop_last=opts.drop_last)
    ...
    def validate(self):
        ...
        if metrics["mIoU"] > self.best_val:
            self.best_val = metrics["mIoU"]
            self.save('best_miou')
        ...
        for epoch in trange(opts.epochs, disable=not opts.viz_epochs):
            ...
            for data_batch in tqdm(self.trainloader, disable=not opts.viz_epochs):
                ...
                with torch.cuda.amp.autocast(enabled=opts.use_amp):
                    ...
                    gt_inst_c = data_batch['inst-centers'].float().cuda()
                    gt_inst_o = data_batch['inst-offsets'].float().cuda()
                ...
                self.scaler1.scale(self.total_loss).backward()
                ...
                with torch.cuda.amp.autocast(enabled=opts.use_amp):
                    ...
                    self.total_unsup_loss = opts.lmbda * loss_ccr
                    ...
                    self.total_unsup_loss += opts.seg_loss_wt * loss_seg
                    ...
                    self.total_unsup_loss += opts.ins_loss_wt * loss_inst

                    # Depth Loss
                    ttn_dep_mask = ttn_deps > 0 
                    loss_dep = self.dep_loss(deps, ttn_deps.detach(), ttn_dep_mask.detach())
                    self.total_unsup_loss += opts.dep_loss_wt * loss_dep
                    train_loss_dep += loss_dep.item()
                    
                # Backward Pass & Update Optimizer
                self.scaler2.scale(self.total_unsup_loss).backward()
